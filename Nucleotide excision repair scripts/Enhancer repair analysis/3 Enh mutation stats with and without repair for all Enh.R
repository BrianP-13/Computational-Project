...
# Enhancer Mutation stats - All enhancers, no chrX ------------------------
...
write.csv(Enh_XR_mutR_table, file = paste0(out,"All enhancers - Excision repair and mutation stats.csv"))

### Plots
# df - Mutation Percentage among all enhancers
ALL_Enh_XR_mutR<-as.data.frame(t(Enh_XR_mutR_table[1,]))
ALL_Enh_XR_mutR$Group<-rownames(ALL_Enh_XR_mutR)
ALL_Enh_XR_mutR<-ALL_Enh_XR_mutR[4:5,]
ALL_Enh_XR_mutR<-melt(ALL_Enh_XR_mutR)
ALL_Enh_XR_mutR$variable<-"mutation\nstatus"

# df - Repair Percentage among all enhancers
ALL_Enh_XR_repair<-Enh_XR_mutR_table
ALL_Enh_XR_repair$Group<-rownames(ALL_Enh_XR_repair)
ALL_Enh_XR_repair<-ALL_Enh_XR_repair[c(4,5,8,9),c(1,6)]
ALL_Enh_XR_repair<-melt(ALL_Enh_XR_repair)
ALL_Enh_XR_repair$variable<-c("6-4PP\nrepair status","6-4PP\nrepair status","CPD\nrepair status","CPD\nrepair status")

# Combine:
ALL_Enh<-rbind(ALL_Enh_XR_mutR,ALL_Enh_XR_repair)
ALL_Enh$Group<-factor(ALL_Enh$Group,levels = c("Percent mutated", "Percent not mutated", "Percent with 6-4PP repair", "Percent without 6-4PP repair", "Percent with CPD repair", "Percent without CPD repair"),
                           labels = c("% with C>T mutations","% with no mutations","% repaired (6-4PP)","% not repaired (6-4PP)","% repaired (CPD)","% not repaired (CPD)"))
ALL_Enh$variable<-factor(ALL_Enh$variable,levels = c("mutation\nstatus","6-4PP\nrepair status","CPD\nrepair status"))

# Plot
yLab<-"Percentage (%)"
ALL_enh_repair_mutR_plot<-ggplot(data = ALL_Enh, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "All enhancers")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","darkolivegreen1","darkolivegreen","firebrick1","firebrick4"))+
  coord_cartesian(ylim = c(0,100))+theme(legend.title = element_blank(),plot.title = element_text(hjust = 0.5),axis.text.x.bottom = element_text(angle = 45, hjust = 1,vjust = 1))
ALL_enh_repair_mutR_plot
ggsave(filename = "Excision_repair_and_mutR_status_for_all_enh.jpeg",
       path = out,plot = ALL_enh_repair_mutR_plot,device = "jpeg",width = 4.5,height = 4,units = "in")


# 6-4PP repair analysis - Top 10% 6-4PP ------------------------------------
...
write.csv(T10_Enh_64PP_XR_mutR_table, file = paste0(out,"T10 64PP enhancers - XR64 repair and mutation stats.csv"))


## Plots
# Mutation Percentage
T10_Enh_XR64_mutR<-as.data.frame(t(T10_Enh_64PP_XR_mutR_table[1,]))
T10_Enh_XR64_mutR$Group<-rownames(T10_Enh_XR64_mutR)
T10_Enh_XR64_mutR<-T10_Enh_XR64_mutR[4:5,]
T10_Enh_XR64_mutR<-melt(T10_Enh_XR64_mutR)
T10_Enh_XR64_mutR$variable<-"mutation\nstatus"

# Repair Percentage
T10_Enh_XR64_repair<-T10_Enh_64PP_XR_mutR_table
T10_Enh_XR64_repair$Group<-rownames(T10_Enh_XR64_repair)
T10_Enh_XR64_repair<-T10_Enh_XR64_repair[c(4,5),c(1,6)]
T10_Enh_XR64_repair<-melt(T10_Enh_XR64_repair)
T10_Enh_XR64_repair$variable<-"6-4PP\nrepair status"

# Combine:
T10_Enh_XR64<-rbind(T10_Enh_XR64_mutR,T10_Enh_XR64_repair)
T10_Enh_XR64$Group<-factor(T10_Enh_XR64$Group,levels = c("Percent mutated", "Percent not mutated","Percent  with repair", "Percent  without repair"),
                           labels = c("% with C>T mutations","% with no mutations","% repaired","% not repaired"))
T10_Enh_XR64$variable<-factor(T10_Enh_XR64$variable,levels = c("mutation\nstatus","6-4PP\nrepair status"))

# Plot
yLab<-"Percentage (%)"
T10_enh_repair_mutR_plot<-ggplot(data = T10_Enh_XR64, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "Enhancers in top\n10% 6-4PP regions")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","darkolivegreen1","darkolivegreen"))+
  coord_cartesian(ylim = c(0,100))+theme(text = element_text(size = 9),axis.text.x.bottom = element_text(angle = 45,hjust = 1,vjust = 1),
                                         plot.title = element_text(hjust = 0.5),legend.title = element_blank())
T10_enh_repair_mutR_plot
ggsave(filename = "T10_64PP_Repair_and_mutR_status.jpeg",
       path = out,plot = T10_enh_repair_mutR_plot,device = "jpeg",width = 4,height = 4,units = "in")
...
write.csv(B10_Enh_64PP_XR_mutR_table, file = paste0(out,"B10 64PP enhancers - XR64 repair and mutation stats.csv"))

## Plots
# Mutation Percentage
B10_Enh_XR64_mutR<-as.data.frame(t(B10_Enh_64PP_XR_mutR_table[1,]))
B10_Enh_XR64_mutR$Group<-rownames(B10_Enh_XR64_mutR)
B10_Enh_XR64_mutR<-B10_Enh_XR64_mutR[4:5,]
B10_Enh_XR64_mutR<-melt(B10_Enh_XR64_mutR)
B10_Enh_XR64_mutR$variable<-"mutation\nstatus"

# Repair Percentage
B10_Enh_XR64_repair<-B10_Enh_64PP_XR_mutR_table
B10_Enh_XR64_repair$Group<-rownames(B10_Enh_XR64_repair)
B10_Enh_XR64_repair<-B10_Enh_XR64_repair[c(4,5),c(1,6)]
B10_Enh_XR64_repair<-melt(B10_Enh_XR64_repair)
B10_Enh_XR64_repair$variable<-"6-4PP\nrepair status"

# Combine:
B10_Enh_XR64<-rbind(B10_Enh_XR64_mutR,B10_Enh_XR64_repair)
B10_Enh_XR64$Group<-factor(B10_Enh_XR64$Group,levels = c("Percent mutated", "Percent not mutated","Percent  with repair", "Percent  without repair"),
                           labels = c("% with C>T mutations","% with no mutations","% repaired","% not repaired"))
B10_Enh_XR64$variable<-factor(B10_Enh_XR64$variable,levels = c("mutation\nstatus","6-4PP\nrepair status"))

# Plot
yLab<-"Percentage (%)"
B10_enh_repair_mutR_plot<-ggplot(data = B10_Enh_XR64, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "Enhancers in bottom\n10% 6-4PP regions")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","darkolivegreen1","darkolivegreen"))+
  coord_cartesian(ylim = c(0,100))+theme(text = element_text(size = 9),axis.text.x.bottom = element_text(angle = 45,hjust = 1,vjust = 1),
                                         plot.title = element_text(hjust = 0.5),legend.title = element_blank())
B10_enh_repair_mutR_plot
ggsave(filename = "B10_64PP_Repair_and_mutR_status.jpeg",
       path = out,plot = B10_enh_repair_mutR_plot,device = "jpeg",width = 4,height = 4,units = "in")
...
write.csv(T10_Enh_CPD_XR_mutR_table, file = paste0(out,"T10 CPD enhancers - XRcpd repair and mutation stats.csv"))

## Plots
# Mutation Percentage
T10_Enh_XRcpd_mutR<-as.data.frame(t(T10_Enh_CPD_XR_mutR_table[1,]))
T10_Enh_XRcpd_mutR$Group<-rownames(T10_Enh_XRcpd_mutR)
T10_Enh_XRcpd_mutR<-T10_Enh_XRcpd_mutR[4:5,]
T10_Enh_XRcpd_mutR<-melt(T10_Enh_XRcpd_mutR)
T10_Enh_XRcpd_mutR$variable<-"mutation\nstatus"

# Repair Percentage
T10_Enh_XRcpd_repair<-T10_Enh_CPD_XR_mutR_table
T10_Enh_XRcpd_repair$Group<-rownames(T10_Enh_XRcpd_repair)
T10_Enh_XRcpd_repair<-T10_Enh_XRcpd_repair[c(4,5),c(1,6)]
T10_Enh_XRcpd_repair<-melt(T10_Enh_XRcpd_repair)
T10_Enh_XRcpd_repair$variable<-"CPD\nrepair status"

# Combine:
T10_Enh_XRcpd<-rbind(T10_Enh_XRcpd_mutR,T10_Enh_XRcpd_repair)
T10_Enh_XRcpd$Group<-factor(T10_Enh_XRcpd$Group,levels = c("Percent mutated","Percent not mutated","Percent  with repair", "Percent  without repair"),
                            labels = c("% with C>T mutations","% with no mutations","% repaired","% not repaired"))
T10_Enh_XRcpd$variable<-factor(T10_Enh_XRcpd$variable,levels = c("mutation\nstatus","CPD\nrepair status"))

# Plots
yLab<-"Percentage (%)"
T10_enh_repair_mutR_plot<-ggplot(data = T10_Enh_XRcpd, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "Enhancers in top\n10% CPD regions")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","firebrick1","firebrick4"))+
  coord_cartesian(ylim = c(0,100))+theme(text = element_text(size = 9),axis.text.x.bottom = element_text(angle = 45,hjust = 1,vjust = 1),
                                         plot.title = element_text(hjust = 0.5),legend.title = element_blank())
T10_enh_repair_mutR_plot
ggsave(filename = "T10_CPD_Repair_and_mutR_status.jpeg",
       path = out,plot = T10_enh_repair_mutR_plot,device = "jpeg",width = 4,height = 4,units = "in")
...
B10_Enh_CPD_XR_mutR_table

write.csv(B10_Enh_CPD_XR_mutR_table, file = paste0(out,"B10 CPD enhancers - XRcpd repair and mutation stats.csv"))


## Plots
# Mutation Percentage
B10_Enh_XRcpd_mutR<-as.data.frame(t(B10_Enh_CPD_XR_mutR_table[1,]))
B10_Enh_XRcpd_mutR$Group<-rownames(B10_Enh_XRcpd_mutR)
B10_Enh_XRcpd_mutR<-B10_Enh_XRcpd_mutR[4:5,]
B10_Enh_XRcpd_mutR<-melt(B10_Enh_XRcpd_mutR)
B10_Enh_XRcpd_mutR$variable<-"mutation\nstatus"

# Repair Percentage
B10_Enh_XRcpd_repair<-B10_Enh_CPD_XR_mutR_table
B10_Enh_XRcpd_repair$Group<-rownames(B10_Enh_XRcpd_repair)
B10_Enh_XRcpd_repair<-B10_Enh_XRcpd_repair[c(4,5),c(1,6)]
B10_Enh_XRcpd_repair<-melt(B10_Enh_XRcpd_repair)
B10_Enh_XRcpd_repair$variable<-"CPD\nrepair status"

# Combine:
B10_Enh_XRcpd<-rbind(B10_Enh_XRcpd_mutR,B10_Enh_XRcpd_repair)
B10_Enh_XRcpd$Group<-factor(B10_Enh_XRcpd$Group,levels = c("Percent mutated","Percent not mutated","Percent  with repair", "Percent  without repair"),
                            labels = c("% with C>T mutations","% with no mutations","% repaired","% not repaired"))
B10_Enh_XRcpd$variable<-factor(B10_Enh_XRcpd$variable,levels = c("mutation\nstatus","CPD\nrepair status"))

# Plot
B10_enh_repair_mutR_plot<-ggplot(data = B10_Enh_XRcpd, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "Enhancers in bottom\n10% CPD regions")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","firebrick1","firebrick4"))+
  coord_cartesian(ylim = c(0,100))+theme(text = element_text(size = 9),axis.text.x.bottom = element_text(angle = 45,hjust = 1,vjust = 1),
                                         plot.title = element_text(hjust = 0.5),legend.title = element_blank())
B10_enh_repair_mutR_plot
ggsave(filename = "B10_CPD_Repair_and_mutR_status.jpeg",
       path = out,plot = B10_enh_repair_mutR_plot,device = "jpeg",width = 4,height = 4,units = "in")
...
write.csv(T10_Enh_Diff_XR64_mutR_table, file = paste0(out,"T10 6-4PP-CPD enhancers - XR64 repair and mutation stats.csv"))


## Plots
# Mutation Percentage
T10_Diff_Enh_XR64_mutR<-as.data.frame(t(T10_Enh_Diff_XR64_mutR_table[1,]))
T10_Diff_Enh_XR64_mutR$Group<-rownames(T10_Diff_Enh_XR64_mutR)
T10_Diff_Enh_XR64_mutR<-T10_Diff_Enh_XR64_mutR[4:5,]
T10_Diff_Enh_XR64_mutR<-melt(T10_Diff_Enh_XR64_mutR)
T10_Diff_Enh_XR64_mutR$variable<-"mutation\nstatus"

# Repair Percentage
T10_Diff_Enh_XR64_repair<-T10_Enh_Diff_XR64_mutR_table
T10_Diff_Enh_XR64_repair$Group<-rownames(T10_Diff_Enh_XR64_repair)
T10_Diff_Enh_XR64_repair<-T10_Diff_Enh_XR64_repair[c(4,5),c(1,6)]
T10_Diff_Enh_XR64_repair<-melt(T10_Diff_Enh_XR64_repair)
T10_Diff_Enh_XR64_repair$variable<-"6-4PP\nrepair status"

# Combine:
T10_Diff_Enh_XR64<-rbind(T10_Diff_Enh_XR64_mutR,T10_Diff_Enh_XR64_repair)
T10_Diff_Enh_XR64$Group<-factor(T10_Diff_Enh_XR64$Group,levels = c("Percent mutated", "Percent not mutated","Percent  with repair","Percent  without repair"),
                                labels = c("% with C>T mutations","% with no mutations","% repaired","% not repaired"))
T10_Diff_Enh_XR64$variable<-factor(T10_Diff_Enh_XR64$variable,levels = c("mutation\nstatus","6-4PP\nrepair status"))

# Plot
yLab<-"Percentage (%)"
T10_Diff_enh_repair_mutR_plot<-ggplot(data = T10_Diff_Enh_XR64, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "Enhancers in top\n10% 6-4PP-CPD regions")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","darkolivegreen1","darkolivegreen"))+
  coord_cartesian(ylim = c(0,100))+theme(text = element_text(size = 9),axis.text.x.bottom = element_text(angle = 45,hjust = 1,vjust = 1),
                                         plot.title = element_text(hjust = 0.5),legend.title = element_blank())
T10_Diff_enh_repair_mutR_plot
ggsave(filename = "T10_6-4PP-CPD_64_Repair_and_mutR_status.jpeg",
       path = out,plot = T10_Diff_enh_repair_mutR_plot,device = "jpeg",width = 4,height = 4,units = "in")
...
write.csv(B10_Enh_Diff_XR64_mutR_table, file = paste0(out,"B10 6-4PP-CPD enhancers - XR64 repair and mutation stats.csv"))


## Plots
# Mutation Percentage
B10_Diff_Enh_XR64_mutR<-as.data.frame(t(B10_Enh_Diff_XR64_mutR_table[1,]))
B10_Diff_Enh_XR64_mutR$Group<-rownames(B10_Diff_Enh_XR64_mutR)
B10_Diff_Enh_XR64_mutR<-B10_Diff_Enh_XR64_mutR[4:5,]
B10_Diff_Enh_XR64_mutR<-melt(B10_Diff_Enh_XR64_mutR)
B10_Diff_Enh_XR64_mutR$variable<-"mutation\nstatus"

# Repair Percentage
B10_Diff_Enh_XR64_repair<-B10_Enh_Diff_XR64_mutR_table
B10_Diff_Enh_XR64_repair$Group<-rownames(B10_Diff_Enh_XR64_repair)
B10_Diff_Enh_XR64_repair<-B10_Diff_Enh_XR64_repair[c(4,5),c(1,6)]
B10_Diff_Enh_XR64_repair<-melt(B10_Diff_Enh_XR64_repair)
B10_Diff_Enh_XR64_repair$variable<-"6-4PP\nrepair status"

# Combine:
B10_Diff_Enh_XR64<-rbind(B10_Diff_Enh_XR64_mutR,B10_Diff_Enh_XR64_repair)
B10_Diff_Enh_XR64$Group<-factor(B10_Diff_Enh_XR64$Group,levels = c("Percent mutated","Percent not mutated","Percent  with repair","Percent  without repair"),
                                labels = c("% with C>T mutations","% with no mutations","% repaired","% not repaired"))
B10_Diff_Enh_XR64$variable<-factor(B10_Diff_Enh_XR64$variable,levels = c("mutation\nstatus","6-4PP\nrepair status"))

# Plot
yLab<-"Percentage (%)"
B10_Diff_enh_repair_mutR_plot<-ggplot(data = B10_Diff_Enh_XR64, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "Enhancers in bottom\n10% 6-4PP-CPD regions")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","darkolivegreen1","darkolivegreen"))+
  coord_cartesian(ylim = c(0,100))+theme(text = element_text(size = 9),axis.text.x.bottom = element_text(angle = 45,hjust = 1,vjust = 1),
                                         plot.title = element_text(hjust = 0.5),legend.title = element_blank())
B10_Diff_enh_repair_mutR_plot
ggsave(filename = "B10_6-4PP-CPD_64_Repair_and_mutR_status.jpeg",
       path = out,plot = B10_Diff_enh_repair_mutR_plot,device = "jpeg",width = 4,height = 4,units = "in")
...
write.csv(T10_Enh_Diff_XRcpd_mutR_table, file = paste0(out,"T10 6-4PP-CPD enhancers - XRcpd repair and mutation stats.csv"))


## Plots
# Mutation Percentage
T10_Diff_Enh_XRcpd_mutR<-as.data.frame(t(T10_Enh_Diff_XRcpd_mutR_table[1,]))
T10_Diff_Enh_XRcpd_mutR$Group<-rownames(T10_Diff_Enh_XRcpd_mutR)
T10_Diff_Enh_XRcpd_mutR<-T10_Diff_Enh_XRcpd_mutR[4:5,]
T10_Diff_Enh_XRcpd_mutR<-melt(T10_Diff_Enh_XRcpd_mutR)
T10_Diff_Enh_XRcpd_mutR$variable<-"mutation\nstatus"

# Repair Percentage
T10_Diff_Enh_XRcpd_repair<-T10_Enh_Diff_XRcpd_mutR_table
T10_Diff_Enh_XRcpd_repair$Group<-rownames(T10_Diff_Enh_XRcpd_repair)
T10_Diff_Enh_XRcpd_repair<-T10_Diff_Enh_XRcpd_repair[c(4,5),c(1,6)]
T10_Diff_Enh_XRcpd_repair<-melt(T10_Diff_Enh_XRcpd_repair)
T10_Diff_Enh_XRcpd_repair$variable<-"CPD\nrepair status"

# Combine:
T10_Diff_Enh_XRcpd<-rbind(T10_Diff_Enh_XRcpd_mutR,T10_Diff_Enh_XRcpd_repair)
T10_Diff_Enh_XRcpd$Group<-factor(T10_Diff_Enh_XRcpd$Group,levels = c("Percent mutated","Percent not mutated","Percent  with repair","Percent  without repair"),
                                 labels = c("% with C>T mutations","% with no mutations","% repaired","% not repaired"))
T10_Diff_Enh_XRcpd$variable<-factor(T10_Diff_Enh_XRcpd$variable,levels = c("mutation\nstatus","CPD\nrepair status"))

# Plot
T10_Diff_enh_repair_mutR_plot<-ggplot(data = T10_Diff_Enh_XRcpd, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "Enhancers in top\n10% 6-4PP-CPD regions")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","firebrick1","firebrick4"))+
  coord_cartesian(ylim = c(0,100))+theme(text = element_text(size = 9),axis.text.x.bottom = element_text(angle = 45,hjust = 1,vjust = 1),
                                         plot.title = element_text(hjust = 0.5),legend.title = element_blank())
T10_Diff_enh_repair_mutR_plot
ggsave(filename = "T10_6-4PP-CPD_CPD_Repair_and_mutR_status.jpeg",
       path = out,plot = T10_Diff_enh_repair_mutR_plot,device = "jpeg",width = 4,height = 4,units = "in")
...
write.csv(B10_Enh_Diff_XRcpd_mutR_table, file = paste0(out,"B10 6-4PP-CPD enhancers - XRcpd repair and mutation stats.csv"))


## Plots
# Mutation Percentage
B10_Diff_Enh_XRcpd_mutR<-as.data.frame(t(B10_Enh_Diff_XRcpd_mutR_table[1,]))
B10_Diff_Enh_XRcpd_mutR$Group<-rownames(B10_Diff_Enh_XRcpd_mutR)
B10_Diff_Enh_XRcpd_mutR<-B10_Diff_Enh_XRcpd_mutR[4:5,]
B10_Diff_Enh_XRcpd_mutR<-melt(B10_Diff_Enh_XRcpd_mutR)
B10_Diff_Enh_XRcpd_mutR$variable<-"mutation\nstatus"

# Repair Percentage
B10_Diff_Enh_XRcpd_repair<-B10_Enh_Diff_XRcpd_mutR_table
B10_Diff_Enh_XRcpd_repair$Group<-rownames(B10_Diff_Enh_XRcpd_repair)
B10_Diff_Enh_XRcpd_repair<-B10_Diff_Enh_XRcpd_repair[c(4,5),c(1,6)]
B10_Diff_Enh_XRcpd_repair<-melt(B10_Diff_Enh_XRcpd_repair)
B10_Diff_Enh_XRcpd_repair$variable<-"CPD\nrepair status"

# Combine:
B10_Diff_Enh_XRcpd<-rbind(B10_Diff_Enh_XRcpd_mutR,B10_Diff_Enh_XRcpd_repair)
B10_Diff_Enh_XRcpd$Group<-factor(B10_Diff_Enh_XRcpd$Group,levels = c("Percent mutated","Percent not mutated","Percent  with repair","Percent  without repair"),
                                 labels = c("% with C>T mutations","% with no mutations","% repaired","% not repaired"))
B10_Diff_Enh_XRcpd$variable<-factor(B10_Diff_Enh_XRcpd$variable,levels = c("mutation\nstatus","CPD\nrepair status"))

# Plot
B10_Diff_enh_repair_mutR_plot<-ggplot(data = B10_Diff_Enh_XRcpd, aes(x = variable,y = value, fill=Group))+
  geom_histogram(stat = "identity",width=0.5)+ theme_bw()+
  ggtitle(label = "Enhancers in bottom\n10% 6-4PP-CPD regions")+xlab(label = NULL)+ylab(yLab)+scale_fill_manual(values=c("dodgerblue","dodgerblue4","firebrick1","firebrick4"))+
  coord_cartesian(ylim = c(0,100))+theme(text = element_text(size = 9),axis.text.x.bottom = element_text(angle = 45,hjust = 1,vjust = 1),
                                         plot.title = element_text(hjust = 0.5),legend.title = element_blank())
B10_Diff_enh_repair_mutR_plot
ggsave(filename = "B10_6-4PP-CPD_CPD_Repair_and_mutR_status.jpeg",
       path = out,plot = B10_Diff_enh_repair_mutR_plot,device = "jpeg",width = 4,height = 4,units = "in")
